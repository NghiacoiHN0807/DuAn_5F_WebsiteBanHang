...
public interface SanphamRepository extends JpaRepository<SanPham, Integer> {
    @Query("SELECT sp, MIN(img.images) " +
            ...
            "WHERE sp.trangThai = :trangThai " +  // Thêm điều kiện trangThai vào đây
            "GROUP BY sp.idSp")
    List<Object[]> getSanPhamWithMinImageUrl(@Param("trangThai") Integer trangThai);
    ...
    @Query(value =
            "SELECT \n" +
                    ...
                    "    GROUP BY id_sp\n" +
                    ") img ON sp.id_sp = img.id_sp\n" +
                    "WHERE ggct.trang_thai = :trangThai", // Add this WHERE clause
            nativeQuery = true)
    Page<Object[]> getSanPhamDetailsByTrangThai(@Param("trangThai") Integer trangThai, Pageable pageable);
